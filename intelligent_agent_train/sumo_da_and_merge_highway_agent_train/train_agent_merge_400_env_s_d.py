...
class D2RLTrainingEnv(core.Env):
    def __init__(self, yaml_conf):
        ...
        self.action_space = spaces.Box(low=-4, high=4, shape=(1,))       # [-4,2] 是加速度 （2，3]是左转  （3，4]是右转
        self.observation_space = spaces.Box(low=-10, high=200, shape=(2,))  # 周围六辆车靠的最近距离那辆车 与自车的相对速度，相对位置
            ...
            if len(time_step_list):
                ...
                for i in range(0, 6):
                    if init_obs[vehs_ids_list[i]]:
                        ...
                        if init_obs[vehs_ids_list[i]]["distance"] < temp_length_two_vehs:
                             nearest_veh_id = vehs_ids_list[i]
                             temp_length_two_vehs = init_obs[vehs_ids_list[i]]["distance"]
                        # six_dis.append(-9)  # 不存在那辆车的话距离-9 代表 无
                # print(six_dis)
                # return np.float32(six_dis)  # 返回周围六辆车的距离作为初始观察
                if nearest_veh_id:
                    relative_speed_and_position.append(abs(init_obs[nearest_veh_id]["velocity"] - init_obs["Ego"]["velocity"]))
                    relative_speed_and_position.append(init_obs[nearest_veh_id]["distance"])
                else:
    ...
    def getreward(self,episode_data,agent_action):   #用主责车辆历史加速度数据和模型得到的
        ...
        for i in range(0,len(agent_action)):
            ...
            if history_action[time_step_list[i]]["Ego"]["prev_action"]:
                prev_action = history_action[time_step_list[i]]["Ego"]["prev_action"]
                if prev_action["lateral"] != "central":     # 变道的奖励更高
                    reward += 1 + abs(prev_action["longitudinal"]-agent_action[i])
                else:
                    reward +=  abs(prev_action["longitudinal"]-agent_action[i])    
                # one_step_history_action = history_action[time_step_list[i]]["Ego"]["prev_action"]["longitudinal"]
                # if one_step_history_action:
                #     reward += abs(one_step_history_action - agent_action[i])
                # else:
                #     reward += abs(0 - agent_action[i])
            else:
                reward = abs(0 - agent_action[i])
...
if __name__ == "__main__":
    with open(
            r"C:\Users\28420\Desktop\SAIC\Dense-Deep-Reinforcement-Learning-main\sumo_da_and_merge_highway_agent_train\train_merge_400_s_d.yaml",
            'r') as f:
