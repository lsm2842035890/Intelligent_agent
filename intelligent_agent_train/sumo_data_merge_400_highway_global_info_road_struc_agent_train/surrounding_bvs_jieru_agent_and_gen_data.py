...
class torch_discriminator_agent:
    def __init__(self, checkpoint_path):
        if not checkpoint_path:
            ...
            checkpoint_path = r"E:\pycharmcode\sumo_data_generation\checkpoints_400m_merge_global_info_struc_Agent\model.pt"
    ...
    def compute_action(self, observation):
        ...
        out = self.model({"obs":obs},[torch.zeros(1,256),torch.zeros(1,256)],torch.tensor([1]))
        # out = self.model(obs)
...
def surrounding_bvs_jieru_agent_gen_data(num_veh,episode_id,episode_only_crash_path):   #把训练好的agent接入到sumo中查看效果，并生成碰撞数据
    ...
    sumo_config_file_path = r"E:\pycharmcode\SAIC\Dense-Deep-Reinforcement-Learning-main\maps\merge_400_highway\merge_400_highway.sumocfg"
    sumo_net_file_path = r"E:\pycharmcode\SAIC\Dense-Deep-Reinforcement-Learning-main\maps\merge_400_highway\merge_400_highway.net.xml"
    ...
    for i in range(0,num_veh):
        if i == int(num_veh/3):     # 让cav位置车流1/3处
            ...
            traci.vehicle.add('CAV', "route_0", typeID='IDM', depart=0, departLane='first', departPos='base', departSpeed=ini_speed,
            ...
            traci.vehicle.moveTo('CAV', f'E0_{random.choice([0,1])}', ini_position)
    ...
    while True:
        ...
        # av周围六辆车用agent控制
        surrounding_bvs = {"Lead":{"veh_id":None,"action":None}, "LeftLead": {"veh_id":None,"action":None}, "RightLead": {"veh_id":None,"action":None}, "Foll": {"veh_id":None,"action":None}, "LeftFoll": {"veh_id":None,"action":None}, "RightFoll": {"veh_id":None,"action":None}}
        surrounding_bvs_with_ego = ["Ego","Lead", "LeftLead", "RightLead", "Foll", "LeftFoll", "RightFoll"]
        # 得到周围六辆车的信息：首先从av视角观察周围有哪些车在，其次得到这些车的信息
        six_vehs_info = [[0 for _ in range(6)] for _ in range(7)]
        av_sur_bvs_info = {}
        if "CAV" in current_vehs:
            av_sur_bvs_info = _get_observation("CAV", time, prev_action=None)   
        # print(av_sur_bvs_info)
        for i in range(0,7):
            if av_sur_bvs_info!={} and av_sur_bvs_info[surrounding_bvs_with_ego[i]] is not None :
                if i != 0:
                    # print(av_sur_bvs_info[surrounding_bvs_with_ego[i]])
                    surrounding_bvs[surrounding_bvs_with_ego[i]]["veh_id"]=av_sur_bvs_info[surrounding_bvs_with_ego[i]]["veh_id"]
                six_vehs_info[i][0] = av_sur_bvs_info[surrounding_bvs_with_ego[i]]["velocity"]
                six_vehs_info[i][1] = av_sur_bvs_info[surrounding_bvs_with_ego[i]]["acceleration"]
                six_vehs_info[i][2] = av_sur_bvs_info[surrounding_bvs_with_ego[i]]["heading"]
                six_vehs_info[i][3] = av_sur_bvs_info[surrounding_bvs_with_ego[i]]["distance"]
                six_vehs_info[i][4] = math.sqrt(av_sur_bvs_info[surrounding_bvs_with_ego[i]]["position"][0]**2+av_sur_bvs_info[surrounding_bvs_with_ego[i]]["position"][1]**2)
                six_vehs_info[i][5] = av_sur_bvs_info[surrounding_bvs_with_ego[i]]["lane_index"]
        # print(six_vehs_info)        
        # 将six_vehs_info变形为合法的模型输入
        agent_input = np.float32(list(itertools.chain(*six_vehs_info)))
        # 得到agent的输出
        ...
        agent_output = agent_output[0][0].tolist()
        # print(agent_output)
        agent_output = [agent_output[i] + agent_output[i + 1] for i in range(0, len(agent_output),2)]
        min_val = min(agent_output)
        max_val = max(agent_output)
        agent_output = [(x - min_val) / (max_val - min_val) for x in agent_output]
        # print(agent_output)
        agent_output = [agent_output[i*4:(i+1)*4] for i in range(6)]
        # print(agent_output)
        # print(surrounding_bvs)
        # 为周围几辆车分配动作
        for i in range(1,7):
            # temp_index :0,1,2,3分别代表左右加减
            temp_index = agent_output[i-1].index(max(agent_output[i-1]))
            if surrounding_bvs[surrounding_bvs_with_ego[i]]["veh_id"] is not None:
                if temp_index == 0:
                    surrounding_bvs[surrounding_bvs_with_ego[i]]["action"]={}
                    surrounding_bvs[surrounding_bvs_with_ego[i]]["action"]["lateral"] = "left"
                    surrounding_bvs[surrounding_bvs_with_ego[i]]["action"]["longitudinal"] = 0
                if temp_index == 1:
                    surrounding_bvs[surrounding_bvs_with_ego[i]]["action"]={}
                    surrounding_bvs[surrounding_bvs_with_ego[i]]["action"]["lateral"] = "right"
                    surrounding_bvs[surrounding_bvs_with_ego[i]]["action"]["longitudinal"] = 0
                if temp_index == 2:
                    surrounding_bvs[surrounding_bvs_with_ego[i]]["action"]={}
                    surrounding_bvs[surrounding_bvs_with_ego[i]]["action"]["lateral"] = "central"
                    surrounding_bvs[surrounding_bvs_with_ego[i]]["action"]["longitudinal"] = random.uniform(0, 2)
                if temp_index == 3:
                    surrounding_bvs[surrounding_bvs_with_ego[i]]["action"]={}
                    surrounding_bvs[surrounding_bvs_with_ego[i]]["action"]["lateral"] = "central"
                    surrounding_bvs[surrounding_bvs_with_ego[i]]["action"]["longitudinal"] = random.uniform(-4,0)

        # 执行代理动作
        choosed_vehs = []
        for i in range(1,7):
            if surrounding_bvs[surrounding_bvs_with_ego[i]]["veh_id"] is not None:
                choosed_veh_id = surrounding_bvs[surrounding_bvs_with_ego[i]]["veh_id"]
                choosed_vehs.append(choosed_veh_id)
                agent_action = surrounding_bvs[surrounding_bvs_with_ego[i]]["action"]
...
if __name__ == "__main__":
    for i in range(0,1):
        surrounding_bvs_jieru_agent_gen_data(15,i,r"E:\pycharmcode\sumo_data_generation\merge_400_highway_with_global_agent_data_episode")
